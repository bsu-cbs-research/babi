...
def _preprocess_capnostream_file(file: str, overwrite: bool = False, export: bool = False):
    """Process a single raw capnostream file into predictable pkl format"""
    ...
    preprocessed_data_path = os.path.join(constants.preprocessed_data_dir, "capnostream", f"{identifier}.pkl")
    print(f"[{identifier}] Preprocessing Capnostream data from {file_path}")

    if not overwrite and os.path.exists(preprocessed_data_path):
        print(f"Preprocessed Capnostream data for {identifier} already exists at {preprocessed_data_path}")
    ...
    if export: # only export if specified
        ...
        if not os.path.exists(os.path.dirname(preprocessed_data_path)):
            os.makedirs(os.path.dirname(preprocessed_data_path))
        df.to_pickle(preprocessed_data_path)
        print(f"[{identifier}] Preprocessed Capnostream data saved to {preprocessed_data_path}")

    print(f"[{identifier}] Preprocessed {len(df)} rows of Capnostream data")
...
def _preprocess_capnostream_files(overwrite: bool = False, export: bool = False):
    ...
    print("Preprocessing all Capnostream files...")
    ...
    for file in [f for f in os.listdir(os.path.join(constants.raw_data_dir, "capnostream")) if f.endswith(".xlsx")]:
        identifier, df = _preprocess_capnostream_file(file, overwrite=overwrite, export=export) # ensure the individual file is preprocessed
    ...
    combined_df.to_pickle(constants.preprocessed_data_paths["capnostream"])

    print(f"Combined preprocessed Capnostream data saved to {constants.preprocessed_data_paths['capnostream']}")

def _preprocess_capnostream_waveform(overwrite: bool = False):
    ...
    df.dropna(how='any', inplace=True) # drop rows that contain NaN values
    df.reset_index(drop=True, inplace=True)  # reset the index after dropping rows

    df.to_pickle(constants.preprocessed_data_paths["waveform"])
    print(f"Preprocessed Capnostream waveform data saved to {constants.preprocessed_data_paths['waveform']}")

def preprocess(overwrite: bool = False):
    """Convert raw capnostream data to structured pkl files then combine them into a single pkl file."""

    if not overwrite and os.path.exists(constants.preprocessed_data_paths["capnostream"]):
        print(f"Preprocessed Capnostream data already exists at {constants.preprocessed_data_paths['capnostream']}")
        return
